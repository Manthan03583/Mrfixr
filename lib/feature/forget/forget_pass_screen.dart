...
class _ForgetPassScreenState extends State<ForgetPassScreen> {
  ...
  void initState() {
    ...
    if (widget.fromVerification) {
      ...
      identityType =
          _configModel.content?.forgetPasswordVerificationMethod ?? "";
    }
  }
                        ...
                        children: [
                          ...
                          if (widget.fromVerification)
                            Padding(
                              ...
                              child: Center(
                                  child: Text(
                                '${"verify_your".tr} ${identityType == "email" ? "email_address".tr.toLowerCase() : "phone_number".tr.toLowerCase()}',
                          ...
                          Padding(
                            ...
                            child: Center(
                                child: Text(
                              '${"please_enter_your".tr} ${identityType == "email" ? "email_address".tr.toLowerCase() : "phone_number".tr.toLowerCase()} ${"to_receive_a_verification_code".tr}',
  ...
  void _forgetPass(
    ...
    if (_identityController.text.isEmpty) {
      ...
      if (widget.fromVerification) {
        authController
            .sendOtpForVerificationScreen(identity, identityType)
            .then((status) {
          if (status.isSuccess!) {
            Get.toNamed(RouteHelper.getVerificationRoute(
                identity, identityType, "verification"));
          } else {
        ...
        authController
            .sendOtpForForgetPassword(identity, identityType)
            .then((status) {
          if (status.isSuccess!) {
            Get.toNamed(RouteHelper.getVerificationRoute(
                identity, identityType, "forget-password"));
          } else {
            customSnackBar(status.message.toString().capitalizeFirst);
          }
        });
