...
class PriceConverter {
  static String getCurrency() {
    ...
    Get.find<SplashController>().configModel.content?.currencies?.forEach((cc) {
      if (cc.code ==
          Get.find<SplashController>().configModel.content?.currencyCode) {
  ...
  static String convertPrice(double? price,
    ...
    if (discount != null && discountType != null) {
      if (discountType == 'amount') {
        ...
        debugPrint('Updated PRICE1-----:${price.toString()}');
      } else if (discountType == 'percent') {
        price = price! - ((discount / 100) * price);
        debugPrint('Updated PRICE2-----:${price.toString()}');
      }
    }
  ...
  static Discount _getDiscount(List<ServiceDiscount>? serviceDiscountList,
      num? discountAmount, String? discountAmountType) {
